...
async def process_item(client, item, llm):
    ...
    for idx, post in enumerate(chain):
        ...
        for url in urls:
            if url not in link_cache:
                ...
                if clean: link_hints.append(f"[Page content: {clean[:400]}]")
        ...
        if idx == 0: root_post = post_data
        elif did == OWNER_DID: relevant_posts.append(post_data)
    thread_id = root_uri
    ...
    if do_search:
        ...
        if search_type == "chainbase": res = await chainbase_search(params.get("query", ""))
        else: res = await tavily_search(params.get("query", ""), params.get("time_range"), params.get("topic"))
        if res: search_results = format_search_result(res, search_type)
    context_parts = []
    for p in reversed(relevant_posts[-5:]):
        txt = p.get("text", "")
        if p.get("link_hints"): txt += "\n" + "\n".join(p["link_hints"])
        if p.get("alts"): txt += "\n" + "\n".join(p["alts"])
        context_parts.append(f"@{p.get('handle', 'unknown')}: {txt}")
    if user_text: context_parts.append(f"[User Question]:\n{user_text}")
    if memory: context_parts.append(f"[Memory]:\n{memory}")
    if search_results: context_parts.append(f"[Search Results]:\n{search_results}")
    if root_post: context_parts.append(f"[ROOT] @{root_post.get('handle', 'unknown')}: {root_post.get('text', '')}")
    full_context = "\n".join(context_parts)
    ...
    final = f"{reply}{sig}"
    if len(final) > 300: final = final[:300].rsplit(' ', 1)[0]
