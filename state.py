    ...
    for i in range(3):
        try:
            with httpx.Client() as c:
                r = c.get(f"https://api.github.com/repos/{GITHUB_REPOSITORY}/actions/secrets/{secret_name}", headers=_headers())
    ...
    for i in range(3):
        try:
            with httpx.Client() as c:
                ...
                r = c.put(f"https://api.github.com/repos/{GITHUB_REPOSITORY}/actions/secrets/{secret_name}", headers=_headers(), json={"encrypted_value": enc, "key_id": kd["key_id"]})
