...
def _extract_embed_full(embed: Optional[Dict]) -> tuple:
    ...
    if embed_type == "app.bsky.embed.images":
        for i, img in enumerate(embed.get("images", []), 1):
            ...
            if alt: parts.append(f"[Image {i}: {alt}]",)
            else: parts.append(f"[Image {i}]")
    elif embed_type == "app.bsky.embed.external":
        ext = embed.get("external", {})
        title = ext.get("title", "").strip()
        desc = ext.get("description", "").strip()
        uri = ext.get("uri", "").strip()
        if title: parts.append(f"[Link: {title}]")
        if desc: parts.append(f"[Desc: {desc[:150]}]")
        if uri and not uri.startswith("https://bsky.app"): parts.append(f"[URL: {uri}]")
        ...
        if rec_type == "app.bsky.feed.post":
            ...
            if quote_text: parts.append(f"[Quote @{quote_author}: {quote_text}]")
        elif rec.get("title"): parts.append(f"[Record: {rec.get('title')}]")
    elif embed_type == "app.bsky.embed.video":
        video = embed.get("video", {})
        alt = video.get("alt", "").strip()
        if alt: parts.append(f"[Video: {alt}]")
        else: parts.append("[Video]")
        ...
        if media_text: parts.append(media_text)
        if record_text: parts.append(record_text)
...
async def _extract_clean_url_content(url: str) -> Optional[str]:
    ...
    except Exception as e: logger.warning(f"Failed to fetch URL {url}: {e}")
    return None
...
async def _parse_thread_nodes(node, parent_uri=None, client=None, token=None, quoted_cache=None, link_cache=None, all_nodes=None):
    ...
    if embed and isinstance(embed, dict):
        ...
        if etype in ["app.bsky.embed.record", "app.bsky.embed.recordWithMedia"]:
            ...
            if rec_ref and rec_ref.get("uri") and rec_ref["uri"] not in quoted_cache:
                ...
                if len(parts) >= 5:
                    try:
                        ...
                        if q.status_code == 200: quoted_cache[rec_ref["uri"]] = q.json().get("value", {}).get("text", "")[:200]
                    except: pass
            ...
            if etype == "app.bsky.embed.recordWithMedia":
                ...
                if media and media.get("$type") == "app.bsky.embed.images":
                    for img in media.get("images", []):
                        if isinstance(img, dict) and img.get("alt"): alts.append(f"@{handle} image: {img['alt']}")
            ...
            desc = ext.get("description", "").strip()
            uri = ext.get("uri", "").strip()
            if title: link_hints.append(f"[Embed Link: {title}]")
            if desc: link_hints.append(f"[Desc: {desc[:150]}]")
            if uri and uri not in link_cache:
                ...
                else: link_cache[uri] = "[Page fetch failed]"
    for url in _extract_urls_from_text(txt):
        if url not in link_cache:
            ...
            if clean:
                ...
                if lm.get("title"): link_hints.append(f"[Linked: {lm['title']}]")
                link_cache[url] = "[Fetch failed]"
    all_nodes.append({
        "uri": node_uri, "parent_uri": parent_uri, "did": did, "handle": handle,
        "text": txt, "alts": alts, "link_hints": link_hints, "is_root": (parent_uri is None)
    })
...
def parse_tavily_results(raw_data: Dict) -> str:
    summary = ""
    if raw_data.get("answer"): summary = f"AI Answer: {raw_data['answer']}\n"
    for res in raw_data.get("results", []):
        text = res.get("raw_content") or res.get("content", "")
        pub_date = res.get("published_date", "")
        if pub_date: text = f"[{pub_date}] {text}"
...
def parse_chainbase_results(raw_data: Dict) -> str:
    ...
    if len(text) > 220:
        ...
        if last_dot > 0: text = text[:last_dot + 1]
        else: text = text[:220].rsplit(' ', 1)[0] + '.'
