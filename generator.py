...
def _extract_text(response) -> str:
    ...
    if isinstance(response, dict):
        ...
        if c and isinstance(c[0], dict): return c[0].get("text", "").strip()
    return ""
...
def add_signature(reply: str, search_type: str = None) -> str:
    ...
    if search_type == "chainbase": return f"{reply}\nQwen | Chainbase"
    return f"{reply}\nQwen"
...
def generate_digest(llm, raw_line: str, max_chars: int = 248) -> str:
    ...
    return clean_artifacts(_extract_text(llm(prompt, max_tokens=120, temperature=0.3)))
...
def get_answer(llm, memory, context, search_results, user_text, do_search, search_type):
    ...
    return clean_artifacts(_extract_text(llm(prompt, max_tokens=MAX_TOKENS, temperature=TEMPERATURE)))

def extract_search_params(llm, user_text, root_text):
    prompt = f"{QUERY_REFINE_SYSTEM}\nUser message: \"{user_text}\"\nContext: \"{root_text}\"\nOutput JSON:"
    try: return json.loads(clean_artifacts(_extract_text(llm(prompt, max_tokens=150, temperature=0.2))))
    except: return {"query": clean_artifacts(user_text), "time_range": "w", "topic": "tech"}

def update_summary(llm, memory, user_text, reply):
    prompt = f"{SUMMARIZE_SYSTEM}\nQ: {user_text}\nA: {reply}\nSummary:"
    return clean_artifacts(_extract_text(llm(prompt, max_tokens=50, temperature=0.3)))
