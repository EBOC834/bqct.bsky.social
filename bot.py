...
async def process_item(client, item, llm):
    ...
    if not thread:
        logger.warning(f"Failed to fetch thread context for {uri}")
        return
...
async def main():
    async with bsky.get_client() as client:
        if not await bsky.login(client, os.getenv("BOT_HANDLE"), os.getenv("BOT_PASSWORD")):
            return
        ...
        if llm:
            ...
            if active_uri and active_uri not in ("{}", "null", ""):
                try:
                    ...
                    logger.error(f"Digest engagement failed: {e}")
            if digest_due:
