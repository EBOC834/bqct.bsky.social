    ...
    try:
        ...
        async with httpx.AsyncClient() as c:
            ...
            if r.status_code in (201, 204):
                ...
                return True
    except Exception as e: logger.error(f"Failed to update secret: {e}")
    return False
...
async def main():
    try:
        ...
        async with httpx.AsyncClient() as client:
            r = await client.post("https://bsky.social/xrpc/com.atproto.server.createSession", json={"identifier": BOT_HANDLE, "password": BOT_PASSWORD}, timeout=10)
            if r.status_code != 200: raise Exception(f"Login failed: {r.status_code}")
            token = r.json()["accessJwt"]
            headers = {"Authorization": f"Bearer {token}"}
            r = await fetch_with_retry(client, "https://bsky.social/xrpc/app.bsky.notification.listNotifications", headers=headers, params={"limit": 20}, timeout=15)
            if r.status_code != 200: raise Exception(f"Fetch failed: {r.status_code}")
            ...
            for n in notifications:
                ...
                if has_t or has_c or f"@{BOT_HANDLE}" in txt or reason == "reply":
                    relevant.append({"uri": uri, "text": txt, "has_search": has_t or has_c, "search_type": "tavily" if has_t else ("chainbase" if has_c else None)})
            if relevant:
                await update_last_processed_secret(latest_idx)
                with open("work_data.json", "w") as f: json.dump({"items": relevant}, f)
            elif notifications:
                await update_last_processed_secret(latest_idx)
